 ...
 * (but I see wikipedia suggests wheat, for some reason) problem, but a quick recap for you:
 ...
 * and etc.
 * Input is always going to be valid/reasonable: ie: a non negative number; extra cookie
 * for not using a loop to compute square-by-square (at least not directly) and instead trying a
...
public class TheWheatRiceAndChessboardProblem {
    ...
    public static int squaresNeededBinaryToString(long grains){
        return grains == 0? 0 : Long.toBinaryString(grains).length();
    }

    public static int squaresNeededLongStream(long grains) {
        return (int) LongStream.iterate(grains, g -> g != 0, g -> g / 2).count();
    }
