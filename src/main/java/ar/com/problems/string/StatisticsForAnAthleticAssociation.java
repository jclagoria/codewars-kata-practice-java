 ...
 *
 * Mean or Average : To calculate mean, add together all of the numbers and then divide
 ...
 * highest value and picking the middle one (e.g., the median of {3, 3, 5, 9, 11} is 5) when there is
 * an odd number of observations. If there is an even number of observations, then there is no single middle value;
...
public class StatisticsForAnAthleticAssociation {
    ...
    /**
     * Compute the statistics for a given string of times.
     *
     * @param strg the string of times to process
     * @return a string containing the range, average and median of the times
     */
    public static String stat(String strg) {
        ...
        // Split the input string into separate times
        List<Integer> times = Arrays.stream(strg.split(",\\s*"))
                .map(StatisticsForAnAthleticAssociation::toSeconds)
                .sorted()
                .toList();

        int sizeTimes = times.size();

        // Calculate the range
        int rangeSec = times.get(sizeTimes - 1) - times.getFirst();

        // Calculate the average
        int sum = times.stream().mapToInt(Integer::intValue).sum();
        int avgSec = sum / sizeTimes;

        // Calculate the median
        int medianSec = (sizeTimes % 2) == 1 ?
                times.get(sizeTimes / 2) :
                (times.get(sizeTimes / 2 - 1) + times.get(sizeTimes / 2)) / 2;
    ...
    /**
     * Converts a time string in the format "h|m|s" to the total number of seconds.
     *
     * @param hms the time string in the format "h|m|s"
     * @return the total number of seconds
     * @throws IllegalArgumentException if the input time string is not in the correct format
     */
    private static int toSeconds(String hms) {
        // Split the input string into hours, minutes, and seconds
        String[] parts = hms.split("\\|");

        // Ensure the input string is in the correct format
        if (parts.length != 3) {
            throw new IllegalArgumentException("Invalid time: " + hms);
        }

        // Parse hours, minutes, and seconds from the split parts
